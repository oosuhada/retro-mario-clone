  ...
  Player.prototype.setAnimation = function() {
	...
	Player.prototype.update = function(dt, vX) {
		...
		if (this.dying){
			...
			this.dying -= 1 * dt;
			if (this.dying <= 0) {
				if (window.canRespawnMario && !window.canRespawnMario()) {
					if (window.onMarioGameOver) window.onMarioGameOver();
					return;
				}
		...
		if (this.exiting) {
			...
			if (this.pos[0] >= this.targetPos[0]) {
				...
				window.setTimeout(function() {
					if (window.onMarioCourseClear) {
						window.onMarioCourseClear();
						return;
					}
	...
	Player.prototype.die = function () {
		if (this.dying) return;
		if (window.onMarioDeath) window.onMarioDeath();
